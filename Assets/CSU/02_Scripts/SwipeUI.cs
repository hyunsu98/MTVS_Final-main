...
public class SwipeUI : MonoBehaviour
{
	...
	[SerializeField]
	private Sprite		circleFillSprite;			// 현재 페이지의 원 이미지
	[SerializeField]
	private Sprite		circleOutlineSprite;		// 현재 페이지가 아닌 원 이미지
	...
	private	float		circleContentScale = 1.2f;  // 현재 페이지의 원 크기(배율)

	public LobbyUIManager lobbyUI_Mgr;
	...
	void test2()
    {
		...
		{
			//if (hitInfo.transform.tag == "SwipeUI") return;
			// 마우스 왼쪽 버튼을 눌렀을 때 1회
			if (Input.GetMouseButtonDown(0))
			{
				// 터치 시작 지점 (Swipe 방향 구분)
	...
	private void UpdateCircleContent()
	{
		...
		for ( int i = 0; i < scrollPageValues.Length; ++ i )
		{
			...
			circleContents[i].GetComponent<Image>().sprite = circleOutlineSprite;
			...
			if ( scrollBar.value < scrollPageValues[i] + (valueDistance / 2) && scrollBar.value > scrollPageValues[i] - (valueDistance / 2) )
			{
				...
				circleContents[i].GetComponent<Image>().sprite = circleFillSprite;
			}
		}
	}
}