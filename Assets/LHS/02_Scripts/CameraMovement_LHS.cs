...
//UI 건들일때 카메라 안움직이기 위해

public class CameraMovement_LHS : MonoBehaviour
{
    [Header("따라갈 오브젝트 정보")]
    ...
    public static CameraMovement_LHS instance;
    ...
    void Start()
    {
        ...
        objectTofollow = GameObject.Find("FollowCam").GetComponent<Transform>();
    ...
    void Update()
    {
        ...
        if(cameraMove == true)
        {
            ...
            if (PublicObjectManager.instance.objectTransformUI.gameObject.activeSelf == true) return;
    ...
    private void LateUpdate()
    {
        ...
        //int layerMask = ((1 << LayerMask.NameToLayer("Player")) / (1 << LayerMask.NameToLayer("Trigger")));
        int layerMask = ((1 << LayerMask.NameToLayer("Wall")));


        // 방해물 존재 시
        // 카메라 위치, FollowCam위치까지, Player를 빼고
        if (Physics.Linecast(savedPos, objectTofollow.position, out hit, layerMask))
        {
            //최종거리는 제한을 둔다
